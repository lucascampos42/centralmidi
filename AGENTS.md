...
- `wp_centralmidi_midis`: `product_id` (UNIQUE), `artista_id`, `genero_id`, `categoria_id`, `mes_lancamento`, `classificacao`, `created_at`, `updated_at`. **Sem strings denormalizadas** (colunas `artista`/`genero` removidas na migração).
